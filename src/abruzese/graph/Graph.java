...
import com.abruzese.graph.edges.EdgeWeight;
 ...
 */
public abstract class Graph<E, W extends EdgeWeight<W>> implements Iterable<E>, Serializable {
     ...
     */
    public abstract Set<Map.Entry<E, E>> getEdges();
    ...
    public void union(Graph<E, W> other) {
        ...
        for (Map.Entry<E, E> edge : other.getEdges()) {
    ...
    protected List<E> search(boolean depth, E source, E destination) {
        ...
        while (!q.isEmpty()) {
            ...
            for (E outboundVertex : getConnected(curPath)) {
                if (!prev.contains(outboundVertex) && !outboundVertex.equals(curPath)) {
                    if (depth) {
        ...
        if (!prev.contains(destination)) return path;
     ...
     */
    public abstract W distance(E from, E to);
